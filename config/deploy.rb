...
set :deploy_to, '/web/rongyun_chatroom'
set :current_path, 'current'
set :repository, 'https://github.com/hengtangan2025/rongyun_chatroom.git'
set :branch, 'master'
set :user, 'root'
set :term_mode, nil
...
task :setup => :environment do
  ...
  order = MinaUtil::Builder.ask_mongoid('rongyun_chatroom')
  ...
  nginx = MinaUtil::Builder.ask_nginx('rongyun_chatroom', '/web/rongyun_chatroom')
  queue! %[echo '#{nginx}' > "/etc/nginx/conf.d/rongyun_chatroom.conf"]
  #test
  #queue! %[echo '#{nginx}' > "/home/dd/rongyun_chatroom.conf"]
